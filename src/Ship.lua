...
return function(ShipTab, Settings, Utils, ShipVisuals, SHIP_TYPES)
    local MobSec = ShipTab:CreateSection("Mobility")
    MobSec:CreateToggle({Name = "Enable Ship Speed", CurrentValue = false, Description = "Boosts your ship's speed (Hold F)", Callback = function(v) Settings.ShipSpeed = v end})
    MobSec:CreateSlider({Name = "Speed Multiplier", Range = {1, 5}, Increment = 0.1, CurrentValue = 1, Description = "Speed multiplier for ship boost", Callback = function(v) Settings.ShipMultiplier = v end})
    
    local VisualSec = ShipTab:CreateSection("Visuals")
    VisualSec:CreateToggle({Name = "Show Ship ESP", CurrentValue = false, Description = "Shows ESP for nearby ships", Callback = function(v) 
        Settings.ShowShipESP = v 
        for _, ship in pairs(ShipVisuals) do ship.label.Visible = v end
    end})
    VisualSec:CreateColorPicker({Name = "Boat Text Color", Color = Settings.ShipColor, Description = "Color of the ship ESP labels", Callback = function(v) Settings.ShipColor = v end})
